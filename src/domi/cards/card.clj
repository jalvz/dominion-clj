...
(def ^{:private true} cards
  [(->Card :copper :treasure 0 1)
   ...
   (->Card :village :kingdom 3 [:speedy :muscle])
   (->Card :workshop :kingdom 3 [:upgrade])
   (->Card :chancellor :kingdom 3 [:cash])
   (->Card :woodcutter :kingdom 3 [:cash])
   (->Card :militia :kingdom 4 [:attack :cash])
   (->Card :remodel :kingdom 4 [:upgrade])
   (->Card :spy :kingdom 4 [:attack])
   (->Card :thief :kingdom 4 [:attack])
   (->Card :bureucrat :kingdom 4 [:attack])
   (->Card :feast :kingdom 4 [:upgrade :trashy])
   (->Card :gardens :kingdom 4 [:victory-ish])
   (->Card :smithy :kingdom 4 [:speedy])
   (->Card :moneylender :kingdom 4 [:trashy :cash])
   (->Card :throne-room :kingdom 4 [])
   (->Card :council-room :kingdom 5 [:speedy])
   (->Card :festival :kingdom 5 [:cash :muscle])
   (->Card :laboratory :kingdom 5 [:speedy])
   (->Card :library :kingdom 5 [:search])
   (->Card :market :kingdom 5 [:speedy])
   (->Card :mine :kingdom 5 [:upgrade])
   (->Card :witch :kingdom 5 [:speedy :attack])
   (->Card :adventurer :kingdom 6 [:search])])
...
(defn cost [cname]
  "Gets the cost of the given card name"
  (get-in card-map [cname :cost]))
